...
public class WordController {
	...
	public int menuPrint(Scanner scan) {
		...
		System.out.println("4. 단어 출력 | 5. 단어 삭제 | 6. 단어 파일로 출력");
	...
	public void addWord() {
		...
		tmp2.add("야옹이");
		tmp2.add("나비");
		Word w2 = new Word("cat", tmp2);
		wordBook.add(w2);
		
		List<String> tmp3 = new ArrayList<>();		
		tmp3.add("돼지");
		tmp3.add("꿀꿀이");
		Word w3 = new Word("pig", tmp3);
		wordBook.add(w3);
	}
	...
	public void insertWord(Scanner scan) { 
		...
		String end = "a";
		while(!end.toLowerCase().equals("y")) // y가 아니면 계속 돌리기
			{ System.out.println("뜻 >> ");
			tmp.add(scan.next());
			System.out.println("y를 누르면 종료됩니다.");
			end = scan.next();
		}
	...
	public void modifyWord(Scanner scan) { 
		...
		int index = wordBook.indexOf(new Word(word));
		if(index == -1) {
			System.out.println("수정 단어가 없습니다.");
			return;
		}
		...
		System.out.println("수정할 뜻 번호를 선택하세요 >>");
		int i = scan.nextInt();
	...
	public void deleteWord(Scanner scan) { 
		...
		if(!wordBook.remove(new Word(word))) {
			System.out.println("삭제 단어가 없습니다.");
			return;
		}
	...
	public void printFile() throws IOException { 
		...
		String fileName = "D:\\web_java_pcy\\memo\\word.txt";
