...
//		System.out.println(hap);
		...
		/* 1. 클래스를 객체화 => new 키워드 사용
		 	  클래스 타입 객체명 = new 클래스(생성자); */
		Method01 me = new Method01();
		int hap2 = me.sum(30, 66);
		System.out.println(hap2);
		int sub = me.sub(3000, 2400);
		System.out.println(sub);
		
		
		/* method = 기능 = 함수 = function
		 ...
		 * method 구조
		   ...
		   - void : 리턴할 값이 없음 -> 리턴타입이 void로 표시
		   - 메서드명 : 소문자로만 시작
		   - 매개변수(= 파라미터) : 해당 기능을 하기 위해 외부에서 반드시 들어와야하는 값 -> 여러개 가능
		   
		 * 메서드는 중복이 안된다 (메서드 안에 메서드가 있을 수 없음)
		 * 메서드 선언 위치
		   - 클래스 안, 다른 메서드 밖
		   - 메서드는 독립적 기능을 함.
		   - 메서드의 순서는 아무 상관이 없음.
